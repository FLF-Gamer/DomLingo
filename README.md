...
当前仓库已经完成 M0 工程骨架、M1 模型配置闭环和 M2 静态正文翻译闭环，下一阶段进入
M3 动态页面、缓存与可靠性开发。详细状态和下一步见[开发路线图](docs/development-roadmap.md)。
...
设置页可以配置和测试模型服务，API Key 使用设备密钥在本机加密保存。当前开发者版本会在
“测试连接并保存”时探测当前 Endpoint 与模型支持的 JSON Schema、JSON
Mode 或 Prompt 兼容模式，并允许将并发请求数设为 1 至 3。它已接通正文检测、翻译、停止
和恢复操作；默认批次限制为约 2,000 字符和 10 个返回 ID，同一段落、列表项或表格行只在
译文完整时整体写回，避免模型截断后出现半句中英混杂。刷新、关闭标签页、跨文档导航或
SPA 路由变化会停止旧会话并禁止迟到响应写回；大页面的正文检测和节点采集会分片让出主线程。
M2 已使用真实模型完成 Chrome 人工验收。并发数为 1 时长页面会明显变慢，日常使用建议设置为
2 或 3。
