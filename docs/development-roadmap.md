...
| --- | --- |
| 更新日期 | 2026-08-08 |
| 当前阶段 | M1 已通过 PR #2 合并，M2 功能开发完成并进行最终验收 |
| 当前里程碑 | M2：静态正文翻译闭环（待最终验收） |
...
| 核心翻译闭环 | 进行中 | 功能开发和自动化回归已完成，等待最终 Chrome 验收与 PR 评审 |
| GitHub Alpha | 待开始 | 依赖核心翻译闭环 |
| Chrome Web Store | 待开始 | 依赖 GitHub Beta 稳定性验证 |

当前总体判断：M0 和 M1 已分别通过 PR #1、PR #2 合并到 `main`。M2 已在独立特性分支和 Draft PR #3 中完成用户点击、按需注入、分片正文检测与节点采集、后台模型请求、严格响应验证、安全写回、失败重试、停止、恢复和页面生命周期取消的静态页面链路。Agent Skills 规范页已使用 Chrome 和真实模型通过核心翻译验收；代码与自动化回归现已收口，下一步只进行新版 Chrome 冒烟验收和 PR 评审。
...
- 保持 Manifest 只使用 `activeTab`、`scripting`、`storage`，生命周期取消没有引入 `tabs` 或 `webNavigation` 权限；
...
- 自动化已覆盖静态 fixture、页面生命周期和 5,000 节点分片扫描，仍需在真实 Chrome 中完成新版生命周期与主线程表现的最终冒烟验收。
...
1. 在 Chrome 中验证刷新、跨文档导航、关闭标签页和 SPA 路由变化会停止旧会话；
2. 在 Chrome 中验证静态文章 fixture 的翻译、停止和恢复；
3. 使用另一篇公开英文文章和真实模型复核翻译质量与 DOM 不变量；
4. 对超大正文页面记录扫描与翻译期间的主线程表现；
5. 补齐验收发现的问题，保持 Draft PR #3 的 CI 通过后进入评审合并。
