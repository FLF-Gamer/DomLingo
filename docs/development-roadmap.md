...
| --- | --- |
| 更新日期 | 2026-08-08 |
| 当前阶段 | M1 已通过 PR #2 合并，M2 静态正文翻译闭环开发中 |
| 当前里程碑 | M2：静态正文翻译闭环（进行中） |
| 当前分支 | `codex/feat/static-translation` |
...
| 核心翻译闭环 | 进行中 | 正文检测、节点事务和模型请求基础已接通 |
| GitHub Alpha | 待开始 | 依赖核心翻译闭环 |
| Chrome Web Store | 待开始 | 依赖 GitHub Beta 稳定性验证 |

当前总体判断：M0 和 M1 已分别通过 PR #1、PR #2 合并到 `main`。M2 已在独立特性分支和 Draft PR #3 中接通用户点击、按需注入、正文检测、分片节点采集、后台模型请求、严格响应验证、安全写回、停止和恢复的静态页面链路，并完成固定页面 fixture 与页面内进度浮层；下一步是在 Chrome 中使用真实模型完成端到端人工验收。
...
- 添加每批 20 个返回 ID 上限、可恢复 Provider 请求重试和节点级分类失败原因；
...
- 尚未监听标签页关闭和顶层文档导航，当前仅显式停止/恢复会主动取消后台请求；
- 正文候选评分仍为同步步骤，需要在 Chrome 超大页面验收中确认单次主线程占用；
- FIX-07 已覆盖协议校验、HTTP 错误、部分批次失败与取消迟到响应，仍需浏览器级故障注入验收；
- Agent Skills 规范页先后出现成功 120/失败 95、成功 78/失败 71，以及成功 27/失败 40；失败主要为整批响应格式错误。现已增加 Provider 结构化输出能力探测、截断识别和有界自适应拆批，仍需在完整刷新原网页并重新测试保存配置后复测；
- 尚未使用 Chrome 和真实模型完成满足退出标准的端到端人工验收。
...
1. 重新加载扩展，在设置页重新“测试连接并保存”以探测结构化输出能力，完整刷新原网页后用相同模型复测 Agent Skills 规范页，并记录探测模式、首轮与手动重试结果；
2. 实现并测试标签页关闭、刷新和跨文档导航时的自动会话取消；
3. 在 Chrome 中验证静态文章 fixture 的翻译、停止和恢复；
4. 使用公开英文文章和真实模型完成翻译质量与 DOM 不变量人工验收；
5. 对超大正文页面记录扫描与翻译期间的主线程表现；
6. 补齐发现的问题，保持 Draft PR #3 的 CI 通过后进入评审合并。
