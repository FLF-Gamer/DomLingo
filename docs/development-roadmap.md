...
当前总体判断：M0 和 M1 已分别通过 PR #1、PR #2 合并到 `main`。M2 已在独立特性分支上接通用户点击、按需注入、正文检测、节点采集、后台模型请求、严格响应验证、安全写回、停止和恢复的第一条静态页面链路；下一步继续补全固定页面 fixture、页面内进度浮层和 Chrome 人工验收。
...
- 添加正文、采集、批处理、响应、会话和 Prompt 单元测试。

当前限制：

- FIX-01、FIX-02、FIX-03 和 FIX-06 已建立，FIX-07 仍需补全更多批次失败场景；
- 超长单节点拆分和分片扫描仍需实现；
- 尚未使用 Chrome 和真实模型对静态文章完成端到端验收。
...
1. 完成 M2 静态页面 fixture 与 DOM 不变量测试；
2. 添加页面内进度、停止和恢复浮层；
3. 在 Chrome 中使用公开英文文章完成真实模型翻译验收；
4. 创建 M2 Draft PR，持续通过 CI 后进入评审合并。
